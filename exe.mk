...
##############################################################################
acefuncs.obj      : $(EXE_SRCS)acefuncs/acefuncs.c         .AUTODEPEND

##############################################################################
commline.obj      : $(EXE_SRCS)commline/commline.c         .AUTODEPEND

##############################################################################
convert.obj       : $(EXE_SRCS)convert/convert.c           .AUTODEPEND

##############################################################################
EXE_INPUT_DIR                     = $(EXE_SRCS)input/#########################

input.obj         : $(EXE_INPUT_DIR)input.c                .AUTODEPEND

inputdos.obj      : $(EXE_INPUT_DIR)dos32.c                .AUTODEPEND

inputnt.obj       : $(EXE_INPUT_DIR)nt.c                   .AUTODEPEND
	$(CC_WIN32)

inputos2.obj      : $(EXE_INPUT_DIR)os2.c                  .AUTODEPEND
	$(CC_OS2)

##############################################################################
MESSAGEDIR                        = $(EXE_SRCS)messages/######################
MSGFILESDIR                       = $(MESSAGEDIR)msgfiles/####################
messages.obj      : $(MESSAGEDIR)messages.c                .AUTODEPEND

##############################################################################
outpterr.obj      : $(EXE_SRCS)outpterr/outpterr.c         .AUTODEPEND

##############################################################################
output.obj        : $(EXE_SRCS)output/output.c             .AUTODEPEND
outdos32.obj      : $(EXE_SRCS)output/dos32.c              .AUTODEPEND
outdosas.obj      : $(EXE_SRCS)output/dos32.asm            .AUTODEPEND
	$(COMPILE_A)
outnt.obj         : $(EXE_SRCS)output/nt.c                 .AUTODEPEND
	$(CC_WIN32)
outos2.obj        : $(EXE_SRCS)output/os2.c                .AUTODEPEND
	$(CC_OS2)

##############################################################################
EXE_STATE_SRCS                    = $(EXE_SRCS)state/#########################
state.obj         : $(EXE_STATE_SRCS)state.c               .AUTODEPEND

statewat.obj      : $(EXE_STATE_SRCS)watcom.c              .AUTODEPEND
